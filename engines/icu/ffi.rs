...
impl Drop for Regex {
    fn drop(&mut self) {
        ...
        unsafe { uregex_close_78(self.ure.as_ptr()) }
    }
}
...
impl Regex {
    ...
    pub fn group_len(&mut self) -> anyhow::Result<usize> {
        ...
        // pointer is valid.
        let mut ec = 0;
        let len = unsafe { uregex_groupCount_78(self.ure.as_ptr(), &mut ec) };
        error_code_check(ec)?;
...
impl<'r, 'h> Matcher<'r, 'h> {
    fn new(
        ...
        unsafe {
            uregex_setText_78(re.ure.as_ptr(), haystack.as_ptr(), len, &mut ec)
        }
    ...
    pub fn find(&mut self) -> anyhow::Result<bool> {
        ...
        // pointer is valid.
        let matched =
            unsafe { uregex_findNext_78(self.re.ure.as_ptr(), &mut ec) };
        error_code_check(ec)?;
        Ok(matched)
    }
    ...
    pub fn start(&mut self, group: usize) -> anyhow::Result<Option<usize>> {
        ...
        // pointer is valid.
        let i =
            unsafe { uregex_start_78(self.re.ure.as_ptr(), group, &mut ec) };
        error_code_check(ec)?;
        if i == -1 {
            Ok(None)
        } else {
    ...
    pub fn end(&mut self, group: usize) -> anyhow::Result<Option<usize>> {
        ...
        // SAFETY: There are no documented preconditions, but we know our regex
        // pointer is valid.
        let i = unsafe { uregex_end_78(self.re.ure.as_ptr(), group, &mut ec) };
        error_code_check(ec)?;
        if i == -1 {
            Ok(None)
        } else {
    ...
    pub fn reset(&mut self) -> anyhow::Result<()> {
        ...
        // pointer is valid.
        unsafe {
            uregex_reset_78(self.re.ure.as_ptr(), 0, &mut ec);
        }
        error_code_check(ec)
    }
}
...
impl<'r, 'h> Drop for Matcher<'r, 'h> {
    fn drop(&mut self) {
        ...
        unsafe {
            const EMPTY: &[u16] = &[];
            uregex_setText_78(self.re.ure.as_ptr(), EMPTY.as_ptr(), 0, &mut ec)
        }
        // There are no documented error conditions, but we check anyway.
...
fn error_code_to_string(ec: UErrorCode) -> anyhow::Result<String> {
    ...
    let name = unsafe { u_errorName_78(ec) };
    anyhow::ensure!(
        !name.is_null(),
...
extern "C" {
    ...
    // regex pattern.
    fn uregex_groupCount_78(
        re: *mut URegularExpression,
        ec: *mut UErrorCode,
    ) -> i32;

    // Utility routines.
    fn u_errorName_78(ec: UErrorCode) -> *const c_char;
}
